...
function MeScroll(a, d) {
  ...
  MeScroll.prototype.extendUpScroll = function (a) {
    ...
    MeScroll.extend(a, {
      ...
      htmlLoading: '<p class="upwarp-progress mescroll-rotate"></p><p class="upwarp-tip"></p>',//加载中 ...
  ...
  class PullRefresh extends React.Component {
      ...
      if (this.props.direction == 'up') {
        _config = {
          ...
          up: {
            page: {
              num: 0,
              size: this.props.pageSize || 5,
              time: null
            },
            htmlNodata: '<p class="upwarp-nodata">—— 没有更多信息 ——</p>',
            auto: false,
            offset: 20,
            isBounce: false,
            callback: this.props.onRefresh || function () {
            } //上拉加载回调,简写callback:function(page){upCallback(page);}
          }
        }
      } else {
        _config = {
          ...
          down: {
            ...
            auto: false,
            offset: this.props.distance || 100,
            callback: this.props.onRefresh || function () {
            } //下拉刷新的回调,别写成downCallback(),多了括号就自动执行方法了
          }
        }
      }
      this.mescroll = new MeScroll(this.props.id, _config);
    }
    ...
    render() {
      return (
        <div id={this.props.id} className="mescroll">
          {this.props.children}
        </div>
      )
    }
